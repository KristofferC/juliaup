...
pub fn run_command_initial_setup_from_launcher() -> Result<()> {
    ...
    let full_version_string = Version::parse(&format!("{}~{}", get_bundled_julia_version(), platform)).unwrap();
    ...
    if path_of_bundled_version.exists() {
        ...
        let mut juliaup_config_data = JuliaupConfig {
            default: Some("release".to_string()),
            installed_versions: HashMap::new(),
            installed_channels: HashMap::new(),
        };

        juliaup_config_data.installed_versions.insert(
        ...
        juliaup_config_data.installed_channels.insert(
            "release".to_string(),
            JuliaupConfigChannel::SystemChannel {
                version: full_version_string.clone(),
            },
        );
        save_config_db(&juliaup_config_data)?;
    } else {
        let mut juliaup_config_data = JuliaupConfig {
            default: Some("release".to_string()),
            installed_versions: HashMap::new(),
        ...
        install_version(&full_version_string, &mut juliaup_config_data, &version_db)?;
